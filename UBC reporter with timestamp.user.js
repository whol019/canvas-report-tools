...
(function () {
  ...
  function addAccessReportButton() {
    ...
    */
    if ($('#jj_ontask_access_report').length === 0) {
      $('#people-options > ul').append('<li class="ui-menu-item" role="presentation" tabindex="-3"><a id="jj_ontask_access_report" class="ui-corner-all" role="menuitem"><i class="icon-analytics"></i> Access data pivot table with timestamp format</a></li>');
  ...
  function getPeople(courseId, url) { //cycles through the student list
    try {
      if (aborted) {
      ...
      $.getJSON(url, function (udata, status, jqXHR) {
        ...
        for (var i = 0; i < udata.length; i++) {
          ...
          try {
            ...
            user.role = user.enrollments[0].role.replace( 'Enrollment', '' );
...
function createOntaskCSV() {
    ...
    for (var id in userData) {
        ...
        ontaskReportAr[id]['role'] = userData[id].role;
